...
def create_2ch_video_prompt(params: Dict) -> str:
    ...
    length_settings = {
        'super_short': '~5分程度の動画になる文字数', 'short': '5-8分程度の動画になる文字数', 'standard': '10-15分程度の動画になる文字数', 'long': '15-20分程度の動画になる文字数', 'super_long': '20分以上の動画になる文字数'
    }
    prompt = f"""
...
- スタイル: {style_settings.get(params.get('style'))}
- 動画長さの目安: {length_settings.get(params.get('length'))}
...
5. **エピローグ（悪役の末路と主人公の未来）**
   - 悪役の悲惨な末路と、平穏を手に入れた主人公の様子を描いて締めくくる。
...
def main():
        ...
        if video_type == "スカッと系動画":
            ...
            with col1:
                ...
                kaigai_style = st.selectbox(
                    ...
                    format_func=lambda x: {'japan_praise': '🇯🇵 日本称賛系', 'technology': '🤖 技術系', 'moving': '💖 感動系', 'anti_china': '⚔️ 嫌中・比較系', 'food': '🍣 食文化・料理系', 'history': '🏯 歴史・伝統系', 'anime_manga': 'ანიメ アニメ・漫画系'}[x],
    ...
    st.markdown("---")
    st.markdown("""<div style="text-align: center; padding: 2rem; color: #666;"><p><strong>Powered by:</strong> Google Gemini API | <strong>Version:</strong> 2.3.1</p></div>""", unsafe_allow_html=True)

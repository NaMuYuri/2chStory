...
"""
    ...
    if params.get('length') in ['long', 'super_long']:
        long_story_instruction = """
【超長文生成のための特別指示】
あなたのモデルには一度に出力できる文章量に上限があることを理解しています。その上限を最大限に活用し、可能な限り長い物語を生成するために、物語を複数の章（例：第一章、第二章...）に分割し、各章で詳細な描写（情景、心理、会話）を豊富に盛り込んでください。章ごとの文字数を意識的に増やすことで、全体として長大で満足度の高い物語を完成させることができます。"""
...
def main():
    ...
    with tab4:
        ...
        with st.expander("📝 高度な物語設定（オプション）"):
            ...
            start_options = {'peaceful': '平穏な日常が、ある出来事をきっかけに崩れ始める。', 'difficult': '主人公が最初から困難な状況に置かれている。', 'mysterious': '謎の出来事や人物が登場し、物語が始まる。'}
            ...
            dev_options = {'escalation': '悪役の嫌がらせがエスカレートしていく。', 'evidence': '主人公が秘密裏に反撃の証拠を集める。', 'misunderstanding': '事態が誤解を招き、より複雑化していく。'}
            ...
            turn_options = {'ally': '強力な助っ人や味方が現れる。', 'limit': '主人公の我慢が限界に達し、覚醒する。', 'mistake': '悪役が決定的なミスを犯す。'}
            ...
            end_options = {'revenge': '悪役は社会的制裁を受け、主人公は幸せになる。', 'forgiveness': '主人公は悪役を許し、新たな一歩を踏み出す。', 'unexpected': '誰も予想しなかった意外な結末を迎える。'}
    ...
    st.markdown("---")
    st.markdown("""<div style="text-align: center; padding: 2rem; color: #666;"><p><strong>Powered by:</strong> Google Gemini API | <strong>Version:</strong> 2.7.0</p></div>""", unsafe_allow_html=True)

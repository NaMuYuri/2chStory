...
def create_long_story_prompt(params: Dict, base_prompt_creator) -> str:
    """長文生成を促すためのラッパープロンプト"""
    base_prompt = base_prompt_creator(params)
    length = params.get('length', 'standard')
    
    if length in ['long', 'super_long']:
        additional_instruction = """
【超長文生成のための特別指示】
あなたのモデルには一度に出力できる文章量に上限があることを理解しています。その上限を最大限に活用し、可能な限り長い物語を生成するために、以下の構造を厳守してください。

1. **物語を5つの章に分割して構成してください。** (第一章: 発端、第二章: 展開、第三章: 転機、第四章: クライマックス、第五章: 結末)
2. **各章ごとに、最低でも1500文字以上、可能であれば2000文字以上を執筆してください。**
3. 各章では、情景描写、人物の心理描写、会話のやり取りを詳細かつ豊富に盛り込んでください。
4. 全体を通して、一貫性を保ちつつ、伏線の設置と回収を意識してください。
...
def create_2ch_video_prompt(params: Dict) -> str:
    """2ch風動画用プロンプト"""
    style_settings = {'love-story': '恋愛','work-life': '職場','school-life': '学校','family': '家族','mystery': '不思議体験','revenge': '復讐','success': '成功体験','heartwarming': 'ほのぼの','shuraba': '修羅場','occult': 'オカルト'}
    prompt = f"""
あなたは人気YouTube動画の台本作家です。
【最重要指示】
これはナレーターが読み上げる「台本」です。2chのスレッドそのものではなく、必ず以下の【出力形式】に従って台本を作成してください。物語は指定された「視点・語り手」から語られるように構成してください。
【設定】
...
def create_kaigai_hanno_prompt(params: Dict) -> str:
    """海外の反応動画用のプロンプト"""
    style_details = {'japan_praise': '日本称賛','technology': '技術','moving': '感動','anti_china': '嫌中・比較','food': '食文化','history': '歴史','anime_manga': 'アニメ・漫画'}
    prompt = f"""
...
def create_sukatto_prompt(params: Dict) -> str:
    """スカッと系動画用のプロンプト"""
    style_details = {'revenge': '復讐劇','dqn_turn': 'DQN返し','karma': '因果応報','workplace': '職場の逆転劇','neighbor': 'ご近所トラブル','in_laws': '嫁姑問題','cheating': '浮気・不倫の制裁'}
    prompt = f"""
あなたは「スカッと系」YouTubeチャンネルのプロの台本作家です。
【最重要指示】
物語は指定された「視点・語り手」から語られるように構成してください。
- 「主人公」視点の場合: 主人公の一人称で物語が進行し、モノローグ（心の声）を多めに含めてください。
- 「第三者ナレーター」視点の場合: 客観的な視点から登場人物の行動や状況を説明してください。
- 「悪役・敵役」視点の場合: 悪役の一人称で物語が進行し、その傲慢な思考を描写してください。
【設定】
...
def main():
    ...
    with tab4:
        ...
        if st.button(f"🚀 {video_type} 台本生成", type="primary", use_container_width=True, key=f"{video_type}_gen"):
            if not video_theme.strip(): st.error("動画テーマを入力してください")
                ...
                # 長文生成用のラッパーをかませる
                final_prompt_func = lambda p: create_long_story_prompt(p, base_prompt_func)
                if generate_content(st.session_state.model, final_prompt_func, params, content_type):
    ...
    with tab6:
        ...
        check_type = st.selectbox(
            ...
            format_func=lambda x: {
                'plot_holes': 'プロットの穴・矛盾チェック',
                'character_consistency': 'キャラクターの一貫性チェック',
                'dialogue_polish': 'セリフの洗練',
                'pacing_improvement': '物語のテンポ改善'
            }[x],
            key="secondary_check_type"
        )
        
        if st.button("📝 二次チェックを実行", type="primary", use_container_width=True, key="secondary_check_button"):
            if not text_to_check_secondary.strip():
                st.error("チェックする文章を入力してください。")
            else:
    ...
    if st.session_state.generated_content:
        ...
        if b_col1.button("🔄 再生成", help="同じ条件で再生成"):
            if st.session_state.last_generation_params:
                ...
                # 長文生成の場合の再生成ロジックを考慮
                prompt_func = params.get('prompt_func')
                if 'length' in params['params'] and params['params']['length'] in ['long', 'super_long']:
                     # 元のベースプロンプト関数を見つける必要があるが、簡略化のため直接呼び出す
                     if "スカッと" in params['content_type']: base_func = create_sukatto_prompt
                     elif "2ch風" in params['content_type']: base_func = create_2ch_video_prompt
                     elif "海外の反応" in params['content_type']: base_func = create_kaigai_hanno_prompt
                     else: base_func = prompt_func
                     final_prompt_func = lambda p: create_long_story_prompt(p, base_func)
                else:
                    final_prompt_func = prompt_func

                if generate_content(st.session_state.model, final_prompt_func, params['params'], params['content_type']):
    ...
    st.markdown("---")
    st.markdown("""<div style="text-align: center; padding: 2rem; color: #666;"><p><strong>Powered by:</strong> Google Gemini API | <strong>Version:</strong> 2.6.0</p></div>""", unsafe_allow_html=True)
